...
import { useCart, CartItem, clearCart } from "@/lib/cartStore";
...
export default function CheckoutPage() {
  ...
  const isSofia = address.city == "Sofia";
  ...
  const shippingCosts = {
    home: isBulgaria ? 3.5 : isAustria ? 4.99 : 9.99,
    courier: 2.5, // ca. 4.99 BGN
    local: 0,
  };

  const shippingCost = deliveryOption ? shippingCosts[deliveryOption] : 0;

  const hasFreeShipping = subtotal >= 50 && deliveryOption !== "local"; // local ist eh gratis
  const newShippingCost = hasFreeShipping ? 0 : shippingCost;

  const total = subtotal + newShippingCost;
  ...
  const handleCheckout = async () => {
    ...
    if (!address.street) {
      setError("Bitte gib die Straße an");
      setLoading(false);
      return;
    }
    if (!address.houseNumber) {
      setError("Bitte gib die Hausnummer an");
      setLoading(false);
      return;
    }
    if (!address.doorNumber) {
      setError("Bitte gib die Türnummer an");
      setLoading(false);
      return;
    }
    if (!address.zip) {
      setError("Bitte gib die PLZ an");
      setLoading(false);
      return;
    }
    if (!address.city) {
      setError("Bitte gib den Ort an");
      setLoading(false);
      return;
    }
    if (!address.city) {
    ...
    if (!deliveryOption) {
      setError("Bitte wähle eine Lieferoption aus");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (deliveryOption === "local")
        status = "Abholung vor Ort - nicht bezahlt";
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAddress({ ...address, doorNumber: e.target.value })
                    }
                    className="w-full px-5 py-4 bg-gray-900 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#e63946]"
                  />
                  ...
                  <label className="flex items-center gap-4 p-5 bg-gray-900 rounded-xl cursor-pointer hover:bg-gray-800 transition">
                    ...
                    <div className="flex-1">
                      <p className="text-lg font-bold">
                        Abholung bei Kurier-Filiale (Speedy/Econt)
                      </p>
                      <p className="text-gray-400 text-sm">
                        {hasFreeShipping ? (
                          <>
